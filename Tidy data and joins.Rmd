...
The tables above give has the head and tail of our new data frame-- mtcarsNew. We can see that all colnames except car in mtcars_add are gathered in "attributte" and the value assigned to them are gathered in "value".  

**A good point about tidyr is that we can gather only a number of columns while the other columns remain the same**  
```{r, chunk 3 gather2, fig.height = 8,fig.width = 12}
