...
```{r,chunk 1 original data,fig.width = 10,fig.height=4}
...
```{r,chunk 2 gather1,fig.width = 10,fig.height=4}
...
The tables above give has the head and tail of our new data frame--GSSvocabNew. We can see that all colnames except gender in   GSSvocab_distinct are gathered in "attributte" and the value assigned to them are gathered in "value".  

**A good point about tidyr is that we can gather only a number of columns while the other columns remain the same**  
```{r, chunk 3 gather2, fig.height = 6,fig.width = 12}
...
```{r, chunk 4 gather3, fig.height=4,fig.width= 10}
...
```{r, chunk 5 spread, fig.height=4, fig.width=10}
...
It returns 0 rows, that is to say the values in  GSSvocab_add and  GSSvocabSpread are matching. And the only difference between the two table above is only the sequence of column.
...
```{r, chunk 7 create a new data,fig.height=4,fig.width=10}
...
```{r, chunk 8 unite,fig.height=4,fig.width=10}
...
```{r, chunk 9 separate, fig.height=4,fig.width=10}
...
```{r, chunk 10 data with missing value,fig.height=4,fig.width=10}
...
```{r, chunk 11 replace,fig.height=4,fig.width=10}
...
```{r, chunk 12 create data without missing value and data with missing value,  fig.width= 10,fig.height=8}
...
***use md.pattern( )***
```{r, chunk 13 visualize missing value}
library( mice )
md.pattern( miss.df )
```
...
```{r, chunk 14 process missing data--delete rows,fig.height=4,fig.width=10}
...
In this part, I use country_codes data set and gapminder data set. country_codes is a dataframe of gapminder country names and ISO 3166-1 country codes

### left_join 

```{r, chunk 15 left join of country_codes and gapminder, fig.height=4,fig.width=10}
library( gapminder )
...
Now, we have a new data frame and the it has all columns appear in country_codes or in gapminder. However, the row number of this new dataframe is 1749, which seems to have no relationship between the row number of gapminder (1704) and that of country_codes which is 187. According to R doucment:  
left_join(x,y):  
return all rows from x, and all columns from x and y. Rows in x with no match in y will have NA values in the new columns. If there are multiple matches between x and y, all combinations of the matches are returned.  
let's check if there is NA values in the new columns
```{r, chunk 16 check if there is NA values in the new columns,fig.height=4,fig.width=10}
...
### right_join 
```{r, chunk 17 right join of gapminder and country_codes,fig.height=4,fig.width=10}
...
As shown in R document: right_join(x,y) returns all rows from y, and all columns from x and y. Rows in y with no match in x will have NA values in the new columns. If there are multiple matches between x and y, all combinations of the matches are returned.   
```{r, chunk 18 check if there is NA values in the new columns of right_cg}
...
Since the all(gapminder_country %in% code_country) returns TRUE, we can make sure that the country name of gapminder is a subset of the country name in country_codes. That is to say, all country names in gapminder have matching values in that of country_codes. So there suppose to be no NA value in right_cg, which can be proved by which(is.na(right_cg)).



### inner_join 
```{r, chunk 19 inner_join of country_codes and gapminder, fig.height=4,fig.width=10}
...
```{r, chunk 20 check if we lose some countries by using inner_join}
...
We lose some rows from country_codes, although these countries appear in country_codes, they are not in gapminder. inner_join( ) only returns infromation of countries which appear in country_codes and gapminder


### full_join 
```{r, chunk 21 full join of gapminder and country_codes, fig.height=4,fig.width=10}
...
full_join(x,y) returns all rows and all columns from both x and y. Where there are not matching values, returns NA for the one missing. In the case where the country name of gapminder is a subset of the country name in country_codes, the full_cg should be same as left_cg:
...
As we ecpected, it returns 0 rows, which means full_cg is same as left_cg.

### semi_join 

```{r, chunk 22 semi_join of country_codes and gapminder, fig.height=4,fig.width=10}
...
We can notice from these two table aboves, semi_join(x,y) returns all rows from x which have matching values in y. sj_cg is a dataframe that has matching values in column country of gapminder and sj_gc is a dataframe which has all rows that have matching values in column country of country_codes.

### anti_join 
```{r, chunk 23 anti_join of country_codes and gapminder, fig.height=4,fig.width=10}
